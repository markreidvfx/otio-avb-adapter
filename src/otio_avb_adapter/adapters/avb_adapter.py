...
def _otio_version_greater_equal(test_version):
    pieces = otio.__version__.split('.')
    otio_major = int(pieces[0])
    otio_minor = int(pieces[1])

    pieces = test_version.split('.')
    test_major = int(pieces[0])
    test_minor = int(pieces[1])

    if otio_major >= test_major and otio_minor >= test_minor:
        return True
    return False
...
def _transcribe(item, parents, edit_rate, indent=0):
    ...
    if isinstance(item, avb.trackgroups.Composition):
        ...
        if not selected_track or isinstance(selected_track.component,
            ...
            if _otio_version_greater_equal("0.15"):
